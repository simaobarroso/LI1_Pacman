...
getPlayermod :: Player -> GhostMode
getPlayermod (Ghost (GhoState (x,y,z,t,h,l) q )) = q

-- | Testa se o Player é pacman ou não (neste caso se é pacman ou ghost)
verificapacman :: Player -- ^ Player que vai ser testado 
               -> Bool -- ^ Verdade caso seja pacman. Falso se não for.
verificapacman (Pacman _)  = True
verificapacman _           = False
